...
@click.option('--make-unique', is_flag=True, help='If supplied, a counter will be appended to a supplied --name if needed to make it unique.')
...
def run(ctx, project, command, name, make_unique, resource_profile, variable):
    ...
    ctx.invoke(job_create, project=project, schedule=None, command=command, name=name,
               ...
               make_unique=False, wait=True, show_run=True, cleanup=True)
